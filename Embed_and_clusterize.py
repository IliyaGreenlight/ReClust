...
def calculate_embedding(model_name: str):
    ...
    with open("comments_with_embeddings.json", "w", encoding="utf-8") as f:
...
def data_prep(all_entries):
    ...
    if len(unique_dims) > 1:
        print(f"⚠️ Warning: inconsistent embedding sizes found: {unique_dims}")
...
def plot_clusters(df, clusters):
    ...
    for i, cluster_id in enumerate(clusters):
        ...
        trace = go.Scattergl(
            ...
            customdata=np.stack([cluster_df["text"], cluster_df["type"], cluster_df["cluster"]], axis=-1),
            hovertemplate=(
    ...
    fig.update_layout(
        ...
        title="YouTube Comments (t-SNE → DBSCAN)",
        width=1100,
        height=800,
        showlegend=False,  # 🚫 no side legend
...
def main(model_name: str = None, eps: float = 1.5, min_samples: int = 8, perplexity: int = 50):
    ...
    df, clusters = make_dataframe_and_clusters(emb_2d, labels, clean_entries)
...
if __name__ == "__main__":
    ...
    MIN_SAMPLES = 5  
    PERPLEXITY = 50 
    main(MODEL_NAME, EPS, MIN_SAMPLES, PERPLEXITY)